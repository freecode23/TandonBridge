...
void merge(vector<int>& v, vector<int>& temp, int start, int mid, int end) {
	...
	// compare left and right partition
	int leftpos = start;
	int rightpos = mid + 1;
	int temppos = start;

	// while left and right "pointer" hasn't reach the end
	while (leftpos <= mid && rightpos <= end) {
		// get the smaller one and insert to temp array
		if (v[leftpos] < v[rightpos])
			temp[temppos++] = v[leftpos++];
		else
			temp[temppos++] = v[rightpos++];
	}
	// get the remaining from left
	while (leftpos <= mid)
		temp[temppos++] = v[leftpos++];

	// get the remainin from right
	while (rightpos <= mid)
		temp[temppos++] = v[rightpos++];
